...
class Site(
    ...
    val sortingRuleGenerator: SortingRuleGenerator? =
        if (sortingRuleConfig != null) SortingRuleGenerator(sortingRuleConfig) else null
    ...
    val redirectUrlGenerator: RedirectUrlGenerator? = if (redirectUrlConfig != null && catalogGenerator != null)
        RedirectUrlGenerator(
            ...
            navigationCatalog.categoryAssignments
        ) else null
    ...
    private inline fun <reified T : Configuration> loadConfig(location: String?): T? {
        ...
        val configFile =
            if (location.startsWith(File.separator) || location.startsWith(".")) // don't touch "complete" paths
                File(location)
            else
